...
export default function TopSystem({
  ...
  const iconButtonClassName = "inline-flex items-center justify-center rounded-2xl border border-black/10 bg-white/70 px-4 py-2 text-black/70 transition hover:bg-black/5"
  const topSystemRef = useRef<HTMLDivElement>(null)
  ...
  useEffect(() => {
    const node = topSystemRef.current
    if (!node) return
  ...
  return (
    <>
      <StickyTopNav nav={NAV} show={showSticky} />

      <header>
        <div ref={topSystemRef} className="relative z-50 bg-[color:var(--bt-bg)]">
          <div className="bt-container py-4">
            ...
            <div className="bt-rail bt-edge">
             ...
             ================================================= */}
              ...
              <div className="bg-[color:var(--bt-bg)]/95 backdrop-blur">
                {/* nav rail */}
                ...
                <div className="border-t border-black/10 bg-[color:var(--bt-breaking)] text-white">
                  <div className="flex items-center gap-3 px-4 py-2">
                    ...
                    <div className="relative flex-1 overflow-hidden">
                      {breaking.length ? (
                        <div className="bt-marquee-wrap">
                          <div className="bt-marquee-track" aria-label="Breaking news ticker">
                            {/* First copy */}
                            {breaking.slice(0, 8).map((p) => (
                              <Link
                                key={`a-${p.fm.slug}`}
                                href={`/news/${p.fm.slug}`}
                                className="mx-4 inline-flex items-center hover:underline"
                              >
                                <span>{p.fm.title}</span>
